-- ============================================================
-- Migration 028: Customer status must not depend on pending fines
-- ============================================================
-- BUG: A customer who had paid every EMI installment (Remaining EMI = 0) but
-- still owed a late fine was being dragged back from COMPLETE to RUNNING by the
-- automatic fine synchronization (recalc_customer_fines / recalc_all_fines, run
-- on the cron endpoint and after every payment). approve_payment_request also
-- refused to mark such a customer COMPLETE in the first place.
--
-- CORRECT BUSINESS RULE:
--   * Customer status depends ONLY on EMI completion.
--       Remaining EMI > 0                    → RUNNING
--       Remaining EMI = 0 (all installs paid) → COMPLETE
--   * A pending fine is surfaced as a "Fine Pending" indicator in the UI but
--     NEVER changes the customer's status. A completed customer with an unpaid
--     fine stays COMPLETE; it must not return to the RUNNING list.
--   * Once the fine is collected the status is unchanged (already COMPLETE).
--
-- This migration:
--   1. Redefines recalc_customer_fines() to keep accruing/persisting fines but
--      REMOVE the "reopen a COMPLETE customer that carries an unpaid fine" block.
--   2. Redefines approve_payment_request() so the auto-complete step no longer
--      treats a pending fine as a blocker (the first-EMI-charge gate, an
--      unrelated rule, is preserved untouched).
--
-- Schema is unchanged. Idempotent & re-runnable (CREATE OR REPLACE).
-- ============================================================

-- ── 1. recalc_customer_fines — accrue fines WITHOUT reopening COMPLETE ────────
...
-- ── 2. approve_payment_request — complete on EMI payment, ignore pending fine ─
