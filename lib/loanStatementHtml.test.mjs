...
const emis = [
  ...
  // Partially paid by Cash — `paid_at` is null (the EMI is not yet cleared);
  // the collection date lives in `partial_paid_at` and the method in `mode`.
  { id: 4, emi_no: 4, due_date: '2025-05-10', amount: 2500, status: 'PARTIALLY_PAID',
    partial_paid_amount: 1000, partial_paid_at: '2025-05-12', mode: 'CASH' },
];
...
function buildArgs() {
  ...
  return {
    ...
    firstDue: emis[0].due_date, lastDue: emis[emis.length - 1].due_date, emiPaidOf,
  };
}
...
test('ledger has one row per EMI plus header + total', () => {
  ...
  assert.equal(bodyRows.length, 4, 'four EMI rows');
  assert.ok(document.querySelector('tfoot'), 'has totals footer');
});

test('paid EMI row shows its "Paid On" date and payment method', () => {
  const html = buildLoanStatementHtml(buildArgs());
  const { document } = new JSDOM(html).window;
  const row1 = document.querySelectorAll('tbody tr')[0];
  // Columns: # | Due | Instalment | Paid | Paid On(4) | Method(5) | Fine(6) | Status | O/S
  assert.match(row1.children[4].textContent, /Feb 25/, 'paid-on date shown');
  assert.doesNotMatch(row1.children[4].textContent, /—/, 'paid-on is not a blank dash');
  assert.match(row1.children[5].textContent, /UPI/, 'payment method shown');
});

test('partially paid EMI shows its date (partial_paid_at) + method, not a dash', () => {
  const html = buildLoanStatementHtml(buildArgs());
  const { document } = new JSDOM(html).window;
  const partialRow = document.querySelectorAll('tbody tr')[3]; // emi_no 4
  // Regression guard: partials carry no `paid_at`, so reading it alone left the
  // date blank. It must fall back to `partial_paid_at`.
  assert.match(partialRow.children[4].textContent, /May 25/, 'partial paid-on date shown');
  assert.doesNotMatch(partialRow.children[4].textContent, /—/, 'partial date is not a blank dash');
  assert.match(partialRow.children[5].textContent, /Cash/, 'partial payment method shown');
});
...
test('PDF carries the full charge-wise breakup (nothing is dropped)', () => {
  const html = buildLoanStatementHtml(buildArgs());
  const { document } = new JSDOM(html).window;
  const text = document.body.textContent;
  assert.match(text, /Charge-wise Breakup/, 'breakup section present');
  assert.match(text, /EMI Principal/);
  assert.match(text, /Late Payment Fine/);
  assert.match(text, /1st EMI Charge/);
});
...
test('totals appear in the document', () => {
  ...
  assert.match(text, /1 \/ 4/, 'EMIs cleared count');
});
